...
(defn- dispatch-to-instance [instance-name [event opts]]
  (re-frame/dispatch [event (if (nil? instance-name)
                              opts
                              (assoc opts :instance-name instance-name))]))

(defn- init [instance-name opts]
  (dispatch-to-instance instance-name [::re-graph/init opts]))
    ...
    (run-test-sync
     ...
     (let [expected-subscription-payload {:id "my-sub"
       ...
       (testing "Subscriptions can be registered"
         ...
         (dispatch [::re-graph/subscribe {:subscription-id :my-sub
                                          :query "{ things { id } }"
                                          :variables {:some "variable"}
         ...
         (testing "and unregistered"
           ...
           (dispatch [::re-graph/unsubscribe {:subscription-id :my-sub}])
  ...
  (let [dispatch (partial dispatch-to-instance instance-name)
    ...
    (run-test-async
     ...
     (testing "websocket reconnects when disconnected"
       ...
       (let [subscription-params {:instance-name (or instance-name default-instance-name)
       ...
       (let [expected-query-payload {:query "query { things { id } }"
         ...
         (testing "In flight queries are deduplicated"
           (let [id :abc-123]
             ...
             (dispatch [::re-graph/query {:query-id id
                                          :query "{ things { id } }"
                                          :variables {:some "variable"}
                                          :callback-event [::on-thing]}])
       ...
       (let [mutation (str "signin($login:String!,$password:String!){"
         ...
         (testing "In flight mutations are deduplicated"
           (let [id :abc-123]
             ...
             (dispatch [::re-graph/mutate {:query-id id
                                           :query mutation
                                           :variables variables
                                           :callback-event [::on-mutate]}])
...
(defn- call-instance [instance-name f]
  (fn [opts]
    (f (if instance-name
         (assoc opts :instance-name instance-name)
         opts))))
...
(defn- run-non-re-frame-test [instance-name]
  (let [db-instance #(get-in @app-db [:re-graph (or instance-name default-instance-name)])
        on-ws-message (on-ws-message (or instance-name default-instance-name))
        init (call-instance instance-name re-graph/init)
        subscribe (call-instance instance-name re-graph/subscribe)
        unsubscribe (call-instance instance-name re-graph/unsubscribe)
        query (call-instance instance-name re-graph/query)
        mutate (call-instance instance-name re-graph/mutate)]
      ...
      (testing "using a websocket"
        (run-test-sync
         ...
         (let [expected-subscription-payload {:id "my-sub"
           ...
           (subscribe {:subscription-id :my-sub
                       :query "{ things { id } }"
                       :variables {:some "variable"}
                       :callback-event callback-fn})
           ...
           (testing "and unregistered"
             ...
             (unsubscribe {:subscription-id :my-sub})
  ...
  (run-test-sync
   ...
   (let [expected-subscription-payload-a {:id "a-sub"
       ...
       (testing "Subscriptions can be registered"
         ...
         (re-frame/dispatch [::re-graph/subscribe {:instance-name :service-a
                                                   :subscription-id :a-sub
                                                   :query "{ things { a } }"
                                                   :variables {:some "a"}
                                                   :callback-event [::on-a-thing]}])
         (re-frame/dispatch [::re-graph/subscribe {:instance-name :service-b
                                                   :subscription-id :b-sub
                                                   :query "{ things { b } }"
                                                   :variables {:some "b"}
                                                   :callback-event [::on-b-thing]}])
         ...
         (testing "and unregistered"
           ...
           (re-frame/dispatch [::re-graph/unsubscribe {:instance-name :service-a :subscription-id :a-sub}])
           (re-frame/dispatch [::re-graph/unsubscribe {:instance-name :service-b :subscription-id :b-sub}])
